...
export const AccomplishmentSection = ({
  resume,
  name,
  bgColor,
}: {
  ...
  return resume?.data?.accomplishment?.data?.length ? (
    <>
    {resume?.data?.accomplishment?.data?.length > 0 &&
    <>
      <View
        wrap={false}
        style={{
      ...
      <View wrap={false} style={styles.lightBlueColor}>
        <View style={styles.coreCompetencies}>
          <View style={{ ...styles.competenciesContent }}>
            {resume?.data?.accomplishment?.data?.map(
