...
const Skills: React.FC<EducationStepProps> = ({ onScreen }) => {
  ...
  const fetchProfileData = async () => {
    ...
    try {
      ...
      setStarItems(response.data.result.data.skill.data || []);
    } catch (err) {
      const errorMessage =
        err instanceof Error
          ? err.message
  ...
  const calculateProgress = (data: any) => {
    ...
    const sections = {
      ...
      workExperience: 25, // Work experience (Companies, Roles, etc.)
      skills: 15, // Skills
    };
  ...
  // Handle edit click
  const handleEdit = (item: StarItem, index: number) => {
                          ...
                          <div className="">
                            ...
                            {isEditModalOpen && selectedItem && (
                              <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-[100]">
                                <div className="bg-white p-6 rounded-lg w-[500px] flex flex-col relative">
                                  <h2>Edit Item</h2>
                                  <input
                                    type="text"
